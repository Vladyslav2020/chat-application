...
const initialState = {
	name: '',
	id: '',
	typing: { status: false, lastTime: 0 },
	chats: [],
	currentChat: { name: '' },
};
...
window.onfocus = () => {
	if (store.status !== 'online') {
		...
		socket.emit('set-last-seen', {
			id: store.id,
			name: store.currentChat.name,
			time: new Date().toISOString(),
		});
		setStore(prevState => {
			localStorage.setItem(
				'chat-data',
				JSON.stringify({ ...prevState, status: 'online' }),
			);
	...
	if (store.status === 'online') {
		setTimeout(() => {
			...
			setStore(prevState => {
				localStorage.setItem(
					'chat-data',
					JSON.stringify({ ...prevState, status: 'offline' }),
				);
	...
	setStore(prevState => {
		prevState.chats.forEach(chat => {
			if (chat.name === data.name) {
				...
				if (
					store.status === 'online' &&
					chat.name === store.currentChat.name
				) {
					socket.emit('set-last-seen', {
						id: store.id,
						name: data.name,
		...
		localStorage.setItem(
			'chat-data',
			JSON.stringify({ ...prevState, chats: [...prevState.chats] }),
		);
	...
	if (store.chats.length > 0) {
		setStore(prevState => {
			localStorage.setItem(
				...
				JSON.stringify({
					...prevState,
					currentChat: { name: store.chats[0].name },
				}),
			);
...
socket.on('delete-user', ({ name }) => {
	setStore(prevState => {
		...
		localStorage.setItem(
			'chat-data',
			JSON.stringify({ ...prevState, chats: filteredChats }),
		);
...
function StoreManager() {
	...
	const chatSwitcher = newActiveChat => {
		...
		setStore(prevState => {
			localStorage.setItem(
				...
				JSON.stringify({
					...prevState,
					currentChat: { name: newActiveChat },
				}),
			);
		...
		if (!store.typing.status) {
			...
			setStore(prevState => {
				localStorage.setItem(
					...
					JSON.stringify({
						...prevState,
						typing: { status: true, lastTime: Date.now() },
					}),
				);
				return {
					...prevState,
	...
	return (
		<React.StrictMode>
			<App
				store={store}
				handlers={{ chatSwitcher, submitMessage, typingHandler }}
			/>
		</React.StrictMode>
	);
}
